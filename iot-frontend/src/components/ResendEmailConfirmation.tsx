...
type IReSendEmailConfirmation = z.infer<typeof reSendEmailConfirmationSchema>;
...
function ResendEmailConfirmation({ open, setOpen }: Props) {
  const { data: companyList } = useGetAllCompany();
  const { data: dealerList } = useGetAllDealer();
  const { data: UserList } = useGetAllUser();
  const { mutate, isLoading, isSuccess } = useReSendEmailConfirmation();
  const {
    handleSubmit,
    reset,
    control,
    formState: { errors },
    clearErrors,
  } = useForm<IReSendEmailConfirmation>({
  ...
  const onSubmit: SubmitHandler<IReSendEmailConfirmation> = (data) => {
    mutate(data);
  };
  return (
    <Dialog

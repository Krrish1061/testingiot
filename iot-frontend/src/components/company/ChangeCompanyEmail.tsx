...
import useChangeCompanyEmail from "../../hooks/company/useChangeCompanyEmail";
...
// backend is not fully implemented
function ChangeCompanyEmail({ email }: Props) {
  const [isEditMode, setIsEditMode] = useState(false);
  const { mutateAsync, isSuccess, isLoading, data } = useChangeCompanyEmail();
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit: SubmitHandler<IFormInputs> = async (data) => {
    if (data.new_email === email) {
      setError("new_email", {
        type: "custom",
        message:
          "Email address is already associated with your Company account",
      });
    } else {
...
export default ChangeCompanyEmail;
